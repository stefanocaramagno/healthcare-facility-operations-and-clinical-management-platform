/**
 * File: frontend/js/patient/payments.js
 *
 * Scopo
 * -----
 * Gestire il comportamento client-side della pagina pagamenti dell’area
 * Patient, comprendendo il caricamento delle prestazioni pagabili,
 * il recupero dello storico dei payment intent, l’applicazione dei filtri,
 * la costruzione del riepilogo di pagamento, l’avvio del pagamento in-app
 * e la gestione delle azioni disponibili sulle operazioni registrate.
 *
 * Ruolo nel sistema
 * -----------------
 * Questo script rappresenta il controller front-end della vista
 * `payments.html`. Coordina l’interazione tra interfaccia utente,
 * servizi API delle aree scheduling e payments e componenti condivisi
 * dell’applicazione, traducendo i dati di appuntamenti e operazioni
 * economiche in una UI consultabile e operativa per il paziente.
 *
 * Responsabilità principali
 * -------------------------
 * - verificare l’accesso alla pagina per il ruolo Patient;
 * - recuperare appuntamenti e payment intent del paziente autenticato;
 ...
 *   - `/api/payments/patients/me/appointments`
 *
 * Note
 * ----
 * Il file è racchiuso in una IIFE per evitare la diffusione di simboli nel
 * global scope. La pagina mantiene uno stato client-side con appuntamenti,
 * payment intent e mappe di lookup per supportare filtro locale, rendering
 * tabellare, riepilogo della prestazione selezionata e azioni rapide
...
(function () {
  ...
  // Endpoint per il recupero dello storico dei payment intent del paziente autenticato.
  const API_INTENTS = "/api/payments/patients/me/intents";

  // Endpoint base per la creazione di un nuovo intent associato a un appuntamento.
  const API_CREATE_INTENT = "/api/payments/patients/me/appointments";

  // Endpoint base per l’avvio o il riavvio dell’elaborazione di un intent esistente.
  const API_PROCESS_INTENT = "/api/payments/patients/me/intents";
  ...
  // il dataset visualizzato o l’avvio di nuove operazioni di pagamento.
  ...
  // Formatta una data API in rappresentazione estesa per riepiloghi e modali.
  ...
  function renderAppointmentSelect(appointments, intentByAppt) {
    ...
    // Ripristina il valore precedente se ancora presente nell’elenco.
    if (current) sel.value = current;

    // Se il valore precedente non è più eleggibile, azzera la selezione.
    if (current && !eligible.some((x) => x.id === current)) sel.value = "";
  }
  ...
  function updatePayButtonState() {
    ...
    // Il pulsante è attivo solo in presenza di una selezione valida e azionabile.
    btn.disabled = !(hasSelection && info.canStart);
  ...
  function renderPaySummary() {
    ...
    let footer = `La richiesta di pagamento sarà inviata al canale digitale.`;
    if (info.status === "PENDING") {
      ...
      footer = `L’ultimo tentativo non è andato a buon fine. È possibile riprovare.`;
  ...
  function renderIntentsTable(intents) {
    ...
    const rows = intents.map((it) => {
      ...
      const canPay = s === "CREATED" || s === "FAILED";
  ...
  // Carica in parallelo appuntamenti e payment intent nel range selezionato,
  ...
  async function loadAll() {
    ...
    try {
      ...
      // Costruisce gli URL completi con i parametri temporali.
      const apptUrl = `${API_APPOINTMENTS}?fromUtc=${encodeURIComponent(rr.fromUtc)}&toUtc=${encodeURIComponent(rr.toUtc)}`;
      const intentsUrl = `${API_INTENTS}?fromUtc=${encodeURIComponent(rr.fromUtc)}&toUtc=${encodeURIComponent(rr.toUtc)}`;
  ...
  async function payForAppointment(appointmentId) {
    ...
    const ok = await confirmAction(
      "Avvia pagamento",
      `Inviare la richiesta di pagamento di ${formatMoney(appt.quotedPriceCents, appt.currency)} al canale digitale?`,
      "Avvia"
    );
    if (!ok) return;
    ...
    try {
      ...
      const created = await apiJson(
        "POST",
        `${API_CREATE_INTENT}/${encodeURIComponent(String(appointmentId))}/intent`,
        { amountCents: null }
      );
      ...
      const processed = await apiJson(
        "POST",
        `${API_PROCESS_INTENT}/${encodeURIComponent(String(intent.id))}/process`,
        { method: "CARD" }
      );
      ...
      if (updated.id) {
        state.intentById.set(String(updated.id), updated);
      }
  ...
  async function retryIntent(intentId) {
    ...
    const ok = await confirmAction(
      "Riprova pagamento",
      "Vuole inviare nuovamente la richiesta di pagamento digitale?",
      "Riprova"
    );
    if (!ok) return;
    ...
    try {
      ...
      const processed = await apiJson(
        "POST",
        `${API_PROCESS_INTENT}/${encodeURIComponent(String(intentId))}/process`,
        { method: "CARD" }
      );
  ...
  function wireEvents() {
    ...
    // La variazione della prestazione selezionata aggiorna il riepilogo e lo stato del pulsante.
    const apptSel = $("appointmentSelect");
    if (apptSel) apptSel.addEventListener("change", () => renderPaySummary());
  ...
  // e mappe di lookup rapido per appuntamenti e payment intent.
  const state = {
    appointments: [],
    intents: [],
    shownIntents: [],
